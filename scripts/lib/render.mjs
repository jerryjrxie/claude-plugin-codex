...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.sessionId) {
    lines.push(`  Claude Code session ID: ${job.sessionId}`);
  }
...
export function renderReviewResult(parsedResult, meta) {
  if (!parsedResult.parsed) {
    const lines = [
      `# Claude Code ${meta.reviewLabel}`,
      "",
      "Claude Code did not return valid structured JSON.",
      "",
  ...
  if (validationError) {
    const lines = [
      ...
      "Claude Code returned JSON with an unexpected review shape.",
      "",
...
export function renderStatusReport(report) {
  ...
  if (report.running.length > 0) {
    ...
    for (const job of report.running) {
      ...
      lines.push(
        `| ${job.id} | ${job.kindLabel} | ${job.status} | ${job.phase ?? ""} | ${job.elapsed ?? ""} | ${job.summary ?? ""} | ${actions.map((a) => `\`${a}\``).join(", ")} |`
      );
    }
    lines.push("");
  }
  ...
  if (report.needsReview) {
    ...
    lines.push("Ending the session will trigger a fresh Claude Code review and block if it finds issues.");
  }
...
export function renderStoredJobResult(job, storedJob) {
  ...
  if (sessionId) {
    lines.push(`Claude Code session ID: ${sessionId}`);
  }
