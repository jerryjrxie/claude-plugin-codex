...
function inferLegacyJobPhase(job, progressPreview = []) {
  ...
  for (let index = progressPreview.length - 1; index >= 0; index -= 1) {
    ...
    if (line.startsWith("starting claude") || line.startsWith("session ready") || line.startsWith("running claude")) {
      return "starting";
    }
    if (line.startsWith("reviewing") || line.includes("review")) {
      return "reviewing";
    }
    if (line.startsWith("running command:")) {
      return "running";
    }
    if (line.startsWith("editing") || line.startsWith("file changes")) {
      return "editing";
    }
    if (line.startsWith("completed") || line.startsWith("finished")) {
      return "finalizing";
    }
...
export function resolveCancelableJob(cwd, reference) {
  ...
  if (activeJobs.length > 1) {
    throw new Error("Multiple Claude Code jobs are active. Pass a job id to $claude-cancel.");
  }
